...
// Elegir avatar, series favoritas y portada del perfil.
            ...
            {pestana === "portada" && (
              <>
                ...
                <input className="input" type="search" value={busca}
                       ...
                       placeholder="🔍  Buscar serie…" style={{ marginBottom: 12 }} />
                <div className="pers-series">
                  <button className={`pers-serie${portada == null ? " elegida" : ""}`}
                          onClick={() => setPort(null)} title="Sin portada">
                    <span className="pers-serie-color" style={{ background: "var(--bg-muted)" }} />
                    <span className="pers-serie-nombre">Ninguna</span>
                  </button>
                  {filtradas.filter(s => s.poster).map(s => (
                    <button key={s.id}
                            className={`pers-serie${portada === s.id ? " elegida" : ""}`}
                            onClick={() => setPort(s.id)}
                            aria-pressed={portada === s.id}
                            title={s.titulo}>
                      <img src={posterTam(s.poster, "w185")} alt="" loading="lazy" />
                      <span className="pers-serie-nombre">{s.titulo}</span>
                    </button>
